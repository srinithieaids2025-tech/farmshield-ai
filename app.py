...
def get_risk_score(probabilities):
    # Weighted score: medium=50 base, high=100 base
    return int(probabilities[0]*10 + probabilities[1]*55 + probabilities[2]*100)
...
<div style='background:#1B4332;padding:24px;border-radius:12px;text-align:center;margin-bottom:20px'>
    ...
    <p style='color:#52B788;margin:6px 0 0 0;font-size:1.1em'>
        Early Crop Disease Forecasting & Yield Optimization
    </p>
    <p style='color:#95D5B2;margin:4px 0 0 0;font-size:0.9em'>
...
with col1:
    crop = st.selectbox("Crop Type", ["Tomato","Rice","Wheat","Cotton","Potato"])
...
if st.button("🔍 Analyse Disease Risk", use_container_width=True):
    ...
    risk_score = get_risk_score(probabilities)
    ...
    <div style='background:#f8f9fa;border:2px solid {border};
        ...
        <h1 style='color:{color};margin:4px 0;font-size:3em'>{risk_score}<span style='font-size:0.4em'>/100</span></h1>
        ...
        <p style='color:#666;font-size:0.9em;margin:4px 0 0 0'>
            Forecast window: next 5–7 days | Crop: {crop} | Stage: {growth_stage}
        </p>
    </div>
    """, unsafe_allow_html=True)
